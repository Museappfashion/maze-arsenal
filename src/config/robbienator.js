...
export function isRobbiePlayerName(playerName) {
  return String(playerName ?? "").trim().toLowerCase() === ROBBIENATOR_PLAYER_NAME;
}
...
export function isRobbienatorWeapon(world, weaponKey = world?.player?.weapon) {
  return hasRobbienatorLoadout(world) && weaponKey === ROBBIENATOR_WEAPON_KEY;
}

export function applyRobbienatorLoadout(world) {
  if (!world || world.labyrinthMode || !isRobbiePlayerName(world.playerName)) {
    return false;
  }
